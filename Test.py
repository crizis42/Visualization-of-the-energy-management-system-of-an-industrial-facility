...
def open_excel_files():
    ...
    try:
        ...
        # Присваиваем каждому GTU его начальные данные
         # Создаем список объектов GTU с данными из Excel
        gtes = []
        for idx, row in gtu_data.iterrows():
            if idx >= 3:  # Пропускаем заголовки
                gtu = GTU(idx-2)  # Нумерация с 1
                gtu.to = initial_to_hours[idx-3]
                gtu.kr = initial_kr_hours[idx-3]
                
                # Читаем даты ремонта из Excel
                if pd.notna(row['Дата вывода в ремонт']):
                    gtu.repair_start = pd.to_datetime(row['Дата вывода в ремонт']).date()
                    gtu.repair_end = pd.to_datetime(row['Плановый срок ремонта']).date()
                    gtu.service_time = (gtu.repair_end - gtu.repair_start).days
                    
                    # Проверяем текущее состояние
                    if gtu.repair_start <= current_date <= gtu.repair_end:
                        gtu.state = 2
                    elif current_date > gtu.repair_end:
                        gtu.state = 0
                        gtu.repair_start = None
                        gtu.repair_end = None
...
class GTU:
    def __init__(self, n):
        ...
        self.repair_start = None  # дата начала ремонта
        self.repair_end = None  # плановая дата окончания ремонта

    def update_state_by_dates(self, current_date):
        if self.repair_start and self.repair_end:
            if self.repair_start <= current_date <= self.repair_end:
                self.state = 2  # В ремонте
            elif current_date > self.repair_end:
                self.state = 0  # Ремонт завершен
                self.service_time = 0
                # Сбрасываем даты ремонта после его завершения
                self.repair_start = None
...
# Модифицируем функцию gtu_initialization для учета дат ремонта
def gtu_initialization():
    ...
    # Обновляем состояния ГТУ по датам ремонта
    for gtu in gtes:
        gtu.update_state_by_dates(current_date)

    # Остальная логика функции остается без изменений
    p_sum = 77.6 if current_season == 'summer' else 89.7
    p_sum += get_power_loss(10, current_season)
    
    n_gtu = math.ceil(p_sum / 16)
    actual_loading = round(p_sum / n_gtu / 16, 2)
    hp = hourly_production(actual_loading)
    engine_hpd = hp * 24
